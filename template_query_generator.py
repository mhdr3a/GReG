...
def highlight_ngrams(sentence, ngrams_to_highlight=[], color='red', highlight_all=False):
    if highlight_all:
...
def main(opt):
    ...
    id_index = None
    if dataset.split('_')[0] == 'IIRC':
        data = iirc_preprocess(data)
    elif dataset.split('_')[0] == 'MuSiQue':
        id_index = 'id'
    ...
    for sample_index in trange(n):
        id = data[sample_index][id_index] if id_index != None else None
        question = data[sample_index]['question']
        question_entities = get_entities(question)
        answer = data[sample_index]['answer']
        if v:
            print(f"Q: {highlight_ngrams(question, question_entities)}")
            print(f"A: {answer}")
            print(highlight_ngrams(f"{model} is responding...", color='green', highlight_all=True))
            tic = time()
        ...
        if v:
            toc = time()
            print(highlight_ngrams(f"response time: {(toc - tic):.2f} s", color='blue', highlight_all=True))
        generated_answer = response.choices[0].message.content
        generated_answer_entities = get_entities(generated_answer)
        if v:
            print(highlight_ngrams(generated_answer, generated_answer_entities))
